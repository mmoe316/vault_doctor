...
-opaque config(SSS) :: {config,
        boolean(),
        boolean(),
        boolean(),
        list({binary(), fun((binary()) -> {ok, SSS} | {error, nil})}),
        list({binary(), gleam@dynamic@decode:decoder(SSS)}),
        list({binary(), gleam@dynamic@decode:decoder(SSS)}),
        boolean(),
        gleam@option:option(fun((binary()) -> SSS)),
        gleam@option:option(SSS),
        gleam@option:option(fun((binary()) -> SSS))}.

-opaque option(SST) :: {option, fun((config(SST)) -> config(SST))}.

-file("src/lustre/component.gleam", 133).
?DOC(false).
-spec new(list(option(SSU))) -> config(SSU).
new(Options) ->
...
-spec on_attribute_change(binary(), fun((binary()) -> {ok, SSY} | {error, nil})) -> option(SSY).
...
-spec on_property_change(binary(), gleam@dynamic@decode:decoder(STC)) -> option(STC).
...
-spec on_context_change(binary(), gleam@dynamic@decode:decoder(STF)) -> option(STF).
...
-spec on_form_autofill(fun((binary()) -> STK)) -> option(STK).
...
-spec on_form_reset(STM) -> option(STM).
on_form_reset(Msg) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_form_restore(fun((binary()) -> STO)) -> option(STO).
...
?DOC(false).
-spec to_server_component_config(config(STW)) -> lustre@runtime@server@runtime:config(STW).
...
-spec default_slot(
    list(lustre@vdom@vattr:attribute(STZ)),
    list(lustre@vdom@vnode:element(STZ))
) -> lustre@vdom@vnode:element(STZ).
...
-spec named_slot(
    binary(),
    list(lustre@vdom@vattr:attribute(SUF)),
    list(lustre@vdom@vnode:element(SUF))
) -> lustre@vdom@vnode:element(SUF).
