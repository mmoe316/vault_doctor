...
-opaque app(TCT, TCU, TCV) :: {app,
        fun((TCT) -> {TCU, lustre@effect:effect(TCV)}),
        fun((TCU, TCV) -> {TCU, lustre@effect:effect(TCV)}),
        fun((TCU) -> lustre@vdom@vnode:element(TCV)),
        lustre@component:config(TCV)}.
...
-type runtime(TCW) :: any() | {gleam_phantom, TCW}.
...
-spec component(
    fun((TDV) -> {TDW, lustre@effect:effect(TDX)}),
    fun((TDW, TDX) -> {TDW, lustre@effect:effect(TDX)}),
    fun((TDW) -> lustre@vdom@vnode:element(TDX)),
    list(lustre@component:option(TDX))
) -> app(TDV, TDW, TDX).
...
-spec application(
    fun((TDM) -> {TDN, lustre@effect:effect(TDO)}),
    fun((TDN, TDO) -> {TDN, lustre@effect:effect(TDO)}),
    fun((TDN) -> lustre@vdom@vnode:element(TDO))
) -> app(TDM, TDN, TDO).
...
-spec element(lustre@vdom@vnode:element(TCZ)) -> app(any(), nil, TCZ).
element(View) ->
    application(
...
-spec simple(
    fun((TDF) -> TDG),
    fun((TDG, TDH) -> TDG),
    fun((TDG) -> lustre@vdom@vnode:element(TDH))
) -> app(TDF, TDG, TDH).
...
-spec do_start(app(TEP, any(), TER), binary(), TEP) -> {ok, runtime(TER)} |
    {error, error()}.
do_start(_, _, _) ->
    {error, not_a_browser}.
...
-spec send(runtime(TFO), lustre@runtime@server@runtime:message(TFO)) -> nil.
send(Runtime, Message) ->
    gleam@erlang@process:send(Runtime, Message).

-file("src/lustre.gleam", 418).
?DOC(
...
-spec dispatch(TFR) -> lustre@runtime@server@runtime:message(TFR).
dispatch(Msg) ->
    {effect_dispatched_message, Msg}.

-file("src/lustre.gleam", 427).
?DOC(
...
-spec start(app(TEG, any(), TEI), binary(), TEG) -> {ok, runtime(TEI)} |
    {error, error()}.
...
-spec start_server_component(app(TEY, any(), TFA), TEY) -> {ok, runtime(TFA)} |
    {error, error()}.
