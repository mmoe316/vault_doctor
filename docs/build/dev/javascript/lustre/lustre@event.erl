...
-spec on(binary(), gleam@dynamic@decode:decoder(UZW)) -> lustre@vdom@vattr:attribute(UZW).
on(Name, Handler) ->
...
-spec advanced(
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UZZ))
) -> lustre@vdom@vattr:attribute(UZZ).
...
-spec handler(VAD, boolean(), boolean()) -> lustre@vdom@vattr:handler(VAD).
...
-spec prevent_default(lustre@vdom@vattr:attribute(VAF)) -> lustre@vdom@vattr:attribute(VAF).
...
-spec stop_propagation(lustre@vdom@vattr:attribute(VAI)) -> lustre@vdom@vattr:attribute(VAI).
...
-spec debounce(lustre@vdom@vattr:attribute(VAL), integer()) -> lustre@vdom@vattr:attribute(VAL).
...
-spec throttle(lustre@vdom@vattr:attribute(VAO), integer()) -> lustre@vdom@vattr:attribute(VAO).
...
-spec on_keypress(fun((binary()) -> VBF)) -> lustre@vdom@vattr:attribute(VBF).
on_keypress(Msg) ->
    on(
        <<"keypress"/utf8>>,
        begin
...
-spec on_keydown(fun((binary()) -> VBH)) -> lustre@vdom@vattr:attribute(VBH).
on_keydown(Msg) ->
    on(
        <<"keydown"/utf8>>,
        begin
...
-spec on_keyup(fun((binary()) -> VBJ)) -> lustre@vdom@vattr:attribute(VBJ).
on_keyup(Msg) ->
    on(
        <<"keyup"/utf8>>,
        begin
...
-spec on_input(fun((binary()) -> VBL)) -> lustre@vdom@vattr:attribute(VBL).
on_input(Msg) ->
    on(
        <<"input"/utf8>>,
        begin
...
-spec on_change(fun((binary()) -> VBN)) -> lustre@vdom@vattr:attribute(VBN).
on_change(Msg) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_check(fun((boolean()) -> VBP)) -> lustre@vdom@vattr:attribute(VBP).
on_check(Msg) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_submit(fun((list({binary(), binary()})) -> VBS)) -> lustre@vdom@vattr:attribute(VBS).
on_submit(Msg) ->
    _pipe@2 = on(
        <<"submit"/utf8>>,
        begin
...
-spec on_focus(VBW) -> lustre@vdom@vattr:attribute(VBW).
on_focus(Msg) ->
    on(<<"focus"/utf8>>, gleam@dynamic@decode:success(Msg)).

-file("src/lustre/event.gleam", 350).
-spec on_blur(VBY) -> lustre@vdom@vattr:attribute(VBY).
on_blur(Msg) ->
