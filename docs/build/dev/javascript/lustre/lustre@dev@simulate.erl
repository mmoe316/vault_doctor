...
-opaque app(TXG, TXH, TXI) :: {app,
        fun((TXG) -> {TXH, lustre@effect:effect(TXI)}),
        fun((TXH, TXI) -> {TXH, lustre@effect:effect(TXI)}),
        fun((TXH) -> lustre@vdom@vnode:element(TXI))}.

-opaque simulation(TXJ, TXK) :: {simulation,
        fun((TXJ, TXK) -> {TXJ, lustre@effect:effect(TXK)}),
        fun((TXJ) -> lustre@vdom@vnode:element(TXK)),
        list(event(TXK)),
        TXJ,
        lustre@vdom@vnode:element(TXK)}.

-type event(TXL) :: {dispatch, TXL} |
...
-spec simple(
    fun((TXM) -> TXN),
    fun((TXN, TXO) -> TXN),
    fun((TXN) -> lustre@vdom@vnode:element(TXO))
) -> app(TXM, TXN, TXO).
...
-spec application(
    fun((TXT) -> {TXU, lustre@effect:effect(TXV)}),
    fun((TXU, TXV) -> {TXU, lustre@effect:effect(TXV)}),
    fun((TXU) -> lustre@vdom@vnode:element(TXV))
) -> app(TXT, TXU, TXV).
...
-spec start(app(TYC, TYD, TYE), TYC) -> simulation(TYD, TYE).
start(App, Args) ->
...
-spec message(simulation(TYK, TYL), TYL) -> simulation(TYK, TYL).
...
-spec problem(simulation(TZQ, TZR), binary(), binary()) -> simulation(TZQ, TZR).
...
-spec model(simulation(TZW, any())) -> TZW.
model(Simulation) ->
    erlang:element(5, Simulation).
...
-spec view(simulation(any(), UAB)) -> lustre@vdom@vnode:element(UAB).
view(Simulation) ->
    erlang:element(6, Simulation).
...
-spec history(simulation(any(), UAG)) -> list(event(UAG)).
history(Simulation) ->
...
-spec click(simulation(TYX, TYY), lustre@dev@query:'query'()) -> simulation(TYX, TYY).
...
-spec input(simulation(TZD, TZE), lustre@dev@query:'query'(), binary()) -> simulation(TZD, TZE).
