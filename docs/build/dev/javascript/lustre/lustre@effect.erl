...
-opaque effect(OFO) :: {effect,
        list(fun((actions(OFO)) -> nil)),
        list(fun((actions(OFO)) -> nil)),
        list(fun((actions(OFO)) -> nil))}.

-type actions(OFP) :: {actions,
        fun((OFP) -> nil),
        fun((binary(), gleam@json:json()) -> nil),
        fun((gleam@erlang@process:selector(OFP)) -> nil),
...
-spec do_comap_select(
    actions(OGZ),
    gleam@erlang@process:selector(OHB),
    fun((OHB) -> OGZ)
) -> nil.
...
-spec do_comap_actions(actions(OGV), fun((OGX) -> OGV)) -> actions(OGX).
...
-spec do_map(list(fun((actions(OGP)) -> nil)), fun((OGP) -> OGS)) -> list(fun((actions(OGS)) -> nil)).
do_map(Effects, F) ->
    gleam@list:map(
        Effects,
        fun(Effect) ->
...
-spec map(effect(OGL), fun((OGL) -> OGN)) -> effect(OGN).
map(Effect, F) ->
    {effect,
...
-spec perform(
    ...
    fun((OHD) -> nil),
    fun((binary(), gleam@json:json()) -> nil),
    fun((gleam@erlang@process:selector(OHD)) -> nil),
    fun(() -> gleam@dynamic:dynamic_()),
    fun((binary(), gleam@json:json()) -> nil)
) -> nil.
...
-spec from(fun((fun((OFS) -> nil)) -> nil)) -> effect(OFS).
from(Effect) ->
    Task = fun(Actions) ->
...
-spec before_paint(fun((fun((OFU) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(OFU).
before_paint(Effect) ->
    Task = fun(Actions) ->
...
-spec after_paint(fun((fun((OFW) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(OFW).
after_paint(Effect) ->
    Task = fun(Actions) ->
        Root = (erlang:element(5, Actions))(),
        Dispatch = erlang:element(2, Actions),
...
?DOC(false).
-spec select(
    fun((fun((OGA) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(OGA))
) -> effect(OGA).
select(Sel) ->
    Task = fun(Actions) ->
...
-spec batch(list(effect(OGH))) -> effect(OGH).
batch(Effects) ->
    gleam@list:fold(
        Effects,
        {effect, [], [], []},
