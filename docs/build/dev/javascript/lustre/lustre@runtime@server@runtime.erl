...
-type state(RMO, RMP) :: {state,
        gleam@erlang@process:subject(message(RMP)),
        gleam@erlang@process:selector(message(RMP)),
        gleam@erlang@process:selector(message(RMP)),
        RMO,
        fun((RMO, RMP) -> {RMO, lustre@effect:effect(RMP)}),
        fun((RMO) -> lustre@vdom@vnode:element(RMP)),
        config(RMP),
        lustre@vdom@vnode:element(RMP),
        lustre@vdom@events:events(RMP),
        gleam@dict:dict(binary(), gleam@json:json()),
        gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(RMP)), gleam@erlang@process:monitor()),
        gleam@set:set(fun((lustre@runtime@transport:client_message(RMP)) -> nil))}.

-type config(RMQ) :: {config,
        boolean(),
        boolean(),
        gleam@dict:dict(binary(), fun((binary()) -> {ok, RMQ} | {error, nil})),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(RMQ)),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(RMQ))}.

-type message(RMR) :: {client_dispatched_message,
        lustre@runtime@transport:server_message()} |
    {client_registered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(RMR))} |
    {client_deregistered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(RMR))} |
    {client_registered_callback,
        fun((lustre@runtime@transport:client_message(RMR)) -> nil)} |
    {client_deregistered_callback,
        fun((lustre@runtime@transport:client_message(RMR)) -> nil)} |
    {effect_added_selector, gleam@erlang@process:selector(message(RMR))} |
    {effect_dispatched_message, RMR} |
...
-file("src/lustre/runtime/server/runtime.gleam", 335).
?DOC(false).
-spec handle_attribute_change(
    gleam@dict:dict(binary(), fun((binary()) -> {ok, RNV} | {error, nil})),
    binary(),
    binary()
) -> {ok, RNV} | {error, nil}.
...
-file("src/lustre/runtime/server/runtime.gleam", 346).
?DOC(false).
-spec handle_property_change(
    gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(ROC)),
    binary(),
...
-file("src/lustre/runtime/server/runtime.gleam", 358).
?DOC(false).
-spec handle_effect(
    gleam@erlang@process:subject(message(ROI)),
    lustre@effect:effect(ROI)
) -> nil.
...
-file("src/lustre/runtime/server/runtime.gleam", 267).
?DOC(false).
...
-file("src/lustre/runtime/server/runtime.gleam", 377).
?DOC(false).
-spec broadcast(
    gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(ROM)), gleam@erlang@process:monitor()),
    gleam@set:set(fun((lustre@runtime@transport:client_message(ROM)) -> nil)),
    lustre@runtime@transport:client_message(ROM)
) -> nil.
...
?DOC(false).
-spec loop(state(RNF, RNG), message(RNG)) -> gleam@otp@actor:next(state(RNF, RNG), message(RNG)).
loop(State, Message) ->
    case Message of
...
?DOC(false).
-spec start(
    {RMV, lustre@effect:effect(RMW)},
    fun((RMV, RMW) -> {RMV, lustre@effect:effect(RMW)}),
    fun((RMV) -> lustre@vdom@vnode:element(RMW)),
    config(RMW)
) -> {ok, gleam@erlang@process:subject(message(RMW))} |
