...
-type diff(QSL) :: {diff, patch(QSL), lustre@vdom@events:events(QSL)}.

-type patch(QSM) :: {patch,
        integer(),
        integer(),
        list(change(QSM)),
        list(patch(QSM))}.

-type change(QSN) :: {replace_text, integer(), binary()} |
    ...
    {replace, integer(), integer(), lustre@vdom@vnode:element(QSN)} |
    {remove, integer(), integer()} |
    {insert, integer(), list(lustre@vdom@vnode:element(QSN)), integer()}.

-file("src/lustre/vdom/patch.gleam", 73).
?DOC(false).
-spec new(integer(), integer(), list(change(QSO)), list(patch(QSO))) -> patch(QSO).
...
?DOC(false).
-spec add_child(patch(QTR), patch(QTR)) -> patch(QTR).
...
?DOC(false).
-spec update(
    list(lustre@vdom@vattr:attribute(QSY)),
    list(lustre@vdom@vattr:attribute(QSY))
) -> change(QSY).
...
?DOC(false).
-spec replace(integer(), lustre@vdom@vnode:element(QTI)) -> change(QTI).
replace(Index, With) ->
    {replace, 5, Index, With}.

-file("src/lustre/vdom/patch.gleam", 123).
?DOC(false).
-spec insert(list(lustre@vdom@vnode:element(QTL)), integer()) -> change(QTL).
