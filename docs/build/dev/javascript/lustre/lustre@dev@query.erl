...
-define(FILEPATH, "src/lustre/dev/query.gleam").
-export([element/1, child/2, descendant/2, 'and'/2, tag/1, namespaced/2, attribute/2, class/1, style/2, id/1, data/2, test_id/1, aria/2, text/1, matches/2, to_readable_string/1, find_all/2, find_path/4, find/2, has/2]).
...
-spec find_matching_in_list(
    ...
    integer()
) -> {ok, {lustre@vdom@vnode:element(TJT), lustre@vdom@path:path()}} |
    {error, nil}.
...
-spec find_direct_child(
    lustre@vdom@vnode:element(TJO),
    selector(),
    lustre@vdom@path:path()
) -> {ok, {lustre@vdom@vnode:element(TJO), lustre@vdom@path:path()}} |
    {error, nil}.
...
-spec find_all_direct_children(lustre@vdom@vnode:element(TKX), selector()) -> list(lustre@vdom@vnode:element(TKX)).
...
-spec find_all_in_list(list(lustre@vdom@vnode:element(TKS)), 'query'()) -> list(lustre@vdom@vnode:element(TKS)).
...
-spec find_all(lustre@vdom@vnode:element(TKK), 'query'()) -> list(lustre@vdom@vnode:element(TKK)).
find_all(Root, Query) ->
    case Query of
        {find_element, Selector} ->
...
-spec find_all_in_children(lustre@vdom@vnode:element(TKO), 'query'()) -> list(lustre@vdom@vnode:element(TKO)).
...
-spec find_all_descendants_in_list(
    list(lustre@vdom@vnode:element(TLK)),
    selector()
) -> list(lustre@vdom@vnode:element(TLK)).
...
-spec find_all_descendants(lustre@vdom@vnode:element(TLG), selector()) -> list(lustre@vdom@vnode:element(TLG)).
...
-spec find_in_list(
    list(lustre@vdom@vnode:element(TJI)),
    'query'(),
    lustre@vdom@path:path(),
    integer()
) -> {ok, {lustre@vdom@vnode:element(TJI), lustre@vdom@path:path()}} |
    {error, nil}.
...
?DOC(false).
-spec find_path(
    lustre@vdom@vnode:element(TIY),
    'query'(),
    integer(),
    lustre@vdom@path:path()
) -> {ok, {lustre@vdom@vnode:element(TIY), lustre@vdom@path:path()}} |
    {error, nil}.
...
-spec find_in_children(
    lustre@vdom@vnode:element(TJD),
    'query'(),
    integer(),
    lustre@vdom@path:path()
) -> {ok, {lustre@vdom@vnode:element(TJD), lustre@vdom@path:path()}} |
    {error, nil}.
...
-spec find(lustre@vdom@vnode:element(TIT), 'query'()) -> {ok,
        lustre@vdom@vnode:element(TIT)} |
    {error, nil}.
find(Root, Query) ->
...
-spec has(lustre@vdom@vnode:element(any()), selector()) -> boolean().
has(Element, Selector) ->
    case find(Element, {find_element, Selector}) of
        {ok, _} ->
            true;
...
-spec find_descendant_in_list(
    list(lustre@vdom@vnode:element(TKE)),
    selector(),
    lustre@vdom@path:path(),
    integer()
) -> {ok, {lustre@vdom@vnode:element(TKE), lustre@vdom@path:path()}} |
    {error, nil}.
...
-spec find_descendant(
    ...
    selector(),
    lustre@vdom@path:path()
) -> {ok, {lustre@vdom@vnode:element(TJZ), lustre@vdom@path:path()}} |
    {error, nil}.
