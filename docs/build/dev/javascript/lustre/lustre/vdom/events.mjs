...
import { isReferenceEqual as is_reference_equal } from "../internals/equals.ffi.mjs";
...
export function decode(events, path, name, event) {
  let $ = $mutable_map.get(
    events.handlers,
...
export function dispatch(events, event) {
...
export function handle(events, path, name, event) {
  let _pipe = decode(events, path, name, event);
  return ((_capture) => { return dispatch(events, _capture); })(_pipe);
}
