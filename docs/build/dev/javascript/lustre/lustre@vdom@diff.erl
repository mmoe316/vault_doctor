...
-type diff(RCD) :: {diff,
        lustre@vdom@patch:patch(RCD),
        lustre@vdom@events:events(RCD)}.

-type attribute_change(RCE) :: {attribute_change,
        list(lustre@vdom@vattr:attribute(RCE)),
        list(lustre@vdom@vattr:attribute(RCE)),
        lustre@vdom@events:events(RCE)}.
...
-file("src/lustre/vdom/diff.gleam", 841).
?DOC(false).
...
-spec diff_attributes(
    ...
    lustre@vdom@events:events(RDG),
    list(lustre@vdom@vattr:attribute(RDG)),
    list(lustre@vdom@vattr:attribute(RDG)),
    list(lustre@vdom@vattr:attribute(RDG)),
    list(lustre@vdom@vattr:attribute(RDG))
) -> attribute_change(RDG).
...
?DOC(false).
-spec do_diff(
    list(lustre@vdom@vnode:element(RCK)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(RCK)),
    list(lustre@vdom@vnode:element(RCK)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(RCK)),
    ...
    integer(),
    lustre@vdom@path:path(),
    list(lustre@vdom@patch:change(RCK)),
    list(lustre@vdom@patch:patch(RCK)),
    fun((gleam@dynamic:dynamic_()) -> gleam@dynamic:dynamic_()),
    lustre@vdom@events:events(RCK)
) -> diff(RCK).
do_diff(
    Old,
    Old_keyed,
