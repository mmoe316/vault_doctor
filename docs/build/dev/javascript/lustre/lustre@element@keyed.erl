...
-spec do_extract_keyed_children(
    list({binary(), lustre@vdom@vnode:element(UII)}),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(UII)),
    list(lustre@vdom@vnode:element(UII))
) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(UII)),
    list(lustre@vdom@vnode:element(UII))}.
...
-spec extract_keyed_children(list({binary(), lustre@vdom@vnode:element(UIA)})) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(UIA)),
    list(lustre@vdom@vnode:element(UIA))}.
...
-spec element(
    binary(),
    list(lustre@vdom@vattr:attribute(UGG)),
    list({binary(), lustre@vdom@vnode:element(UGG)})
) -> lustre@vdom@vnode:element(UGG).
...
-spec namespaced(
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(UGM)),
    list({binary(), lustre@vdom@vnode:element(UGM)})
) -> lustre@vdom@vnode:element(UGM).
...
-spec fragment(list({binary(), lustre@vdom@vnode:element(UGS)})) -> lustre@vdom@vnode:element(UGS).
fragment(Children) ->
    {Keyed_children, Children@1} = extract_keyed_children(Children),
...
-spec ul(
    list(lustre@vdom@vattr:attribute(UGW)),
    list({binary(), lustre@vdom@vnode:element(UGW)})
) -> lustre@vdom@vnode:element(UGW).
ul(Attributes, Children) ->
    element(<<"ul"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 148).
-spec ol(
    list(lustre@vdom@vattr:attribute(UHC)),
    list({binary(), lustre@vdom@vnode:element(UHC)})
) -> lustre@vdom@vnode:element(UHC).
ol(Attributes, Children) ->
    element(<<"ol"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 155).
-spec 'div'(
    list(lustre@vdom@vattr:attribute(UHI)),
    list({binary(), lustre@vdom@vnode:element(UHI)})
) -> lustre@vdom@vnode:element(UHI).
'div'(Attributes, Children) ->
    element(<<"div"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 162).
-spec tbody(
    list(lustre@vdom@vattr:attribute(UHO)),
    list({binary(), lustre@vdom@vnode:element(UHO)})
) -> lustre@vdom@vnode:element(UHO).
tbody(Attributes, Children) ->
    element(<<"tbody"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 169).
-spec dl(
    list(lustre@vdom@vattr:attribute(UHU)),
    list({binary(), lustre@vdom@vnode:element(UHU)})
) -> lustre@vdom@vnode:element(UHU).
