...
-export([merge/2, compare/2, prepare/1, attribute/2, to_string_tree/3, property/2, event/7, to_json/1]).
...
-type attribute(NGX) :: {attribute, integer(), binary(), binary()} |
    {property, integer(), binary(), gleam@json:json()} |
    {event,
        integer(),
        binary(),
        gleam@dynamic@decode:decoder(handler(NGX)),
        list(binary()),
        event_behaviour(),
        event_behaviour(),
        integer(),
...
-type handler(NGY) :: {handler, boolean(), boolean(), NGY}.
...
?DOC(false).
-spec merge(list(attribute(NHN)), list(attribute(NHN))) -> list(attribute(NHN)).
...
?DOC(false).
-spec compare(attribute(NHU), attribute(NHU)) -> gleam@order:order().
compare(A, B) ->
    gleam@string:compare(erlang:element(3, A), erlang:element(3, B)).

-file("src/lustre/vdom/vattr.gleam", 91).
?DOC(false).
-spec prepare(list(attribute(NHI))) -> list(attribute(NHI)).
prepare(Attributes) ->
    case Attributes of
        [] ->
            Attributes;
...
-spec event(
    binary(),
    gleam@dynamic@decode:decoder(handler(NHD)),
    list(binary()),
    event_behaviour(),
    event_behaviour(),
    integer(),
