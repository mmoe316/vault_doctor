...
-spec map({ok, CME} | {error, CMF}, fun((CME) -> CMI)) -> {ok, CMI} |
    {error, CMF}.
map(Result, Fun) ->
    case Result of
        {ok, X} ->
...
-spec map_error({ok, CML} | {error, CMM}, fun((CMM) -> CMP)) -> {ok, CML} |
    {error, CMP}.
...
-spec flatten({ok, {ok, CMS} | {error, CMT}} | {error, CMT}) -> {ok, CMS} |
    {error, CMT}.
flatten(Result) ->
    case Result of
        {ok, X} ->
...
-spec 'try'({ok, CNA} | {error, CNB}, fun((CNA) -> {ok, CNE} | {error, CNB})) -> {ok,
        CNE} |
    {error, CNB}.
'try'(Result, Fun) ->
    case Result of
...
-spec then({ok, CNJ} | {error, CNK}, fun((CNJ) -> {ok, CNN} | {error, CNK})) -> {ok,
        CNN} |
    {error, CNK}.
then(Result, Fun) ->
    'try'(Result, Fun).
...
-spec unwrap({ok, CNS} | {error, any()}, CNS) -> CNS.
...
-spec lazy_unwrap({ok, CNW} | {error, any()}, fun(() -> CNW)) -> CNW.
...
-spec unwrap_error({ok, any()} | {error, COB}, COB) -> COB.
...
-spec unwrap_both({ok, COE} | {error, COE}) -> COE.
unwrap_both(Result) ->
    case Result of
        {ok, A} ->
            A;
...
-spec 'or'({ok, COH} | {error, COI}, {ok, COH} | {error, COI}) -> {ok, COH} |
    {error, COI}.
'or'(First, Second) ->
    case First of
        {ok, _} ->
...
-spec lazy_or({ok, COP} | {error, COQ}, fun(() -> {ok, COP} | {error, COQ})) -> {ok,
        COP} |
    {error, COQ}.
...
-spec all(list({ok, COX} | {error, COY})) -> {ok, list(COX)} | {error, COY}.
all(Results) ->
    gleam@list:try_map(Results, fun(Result) -> Result end).

-file("src/gleam/result.gleam", 353).
-spec partition_loop(list({ok, CPM} | {error, CPN}), list(CPM), list(CPN)) -> {list(CPM),
    list(CPN)}.
...
-spec partition(list({ok, CPF} | {error, CPG})) -> {list(CPF), list(CPG)}.
partition(Results) ->
...
-spec replace({ok, any()} | {error, CPV}, CPY) -> {ok, CPY} | {error, CPV}.
...
-spec replace_error({ok, CQB} | {error, any()}, CQF) -> {ok, CQB} | {error, CQF}.
...
-spec values(list({ok, CQI} | {error, any()})) -> list(CQI).
values(Results) ->
